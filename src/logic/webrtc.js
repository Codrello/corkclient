...
export class WebRTC extends Emitter {
    ...
    static async checkStatus() {
        return new Promise((resolve, reject) => {
            ...
            channel.on('connect', () => {
                channel.postMessage(JSON.stringify({ name: 'status', data: { ping: id } }));
            });
    ...
    channelEmit(name, data) {
        this.websocketChannel.postMessage(JSON.stringify({ name, data }));
    }
    constructor({ wrtc = undefined, // wrtc is used for unit testing via node.js
    room = undefined, peerSettings = {}, } = {}) {
        super();
    ...
    updateStatus() {
        const status = Object.values(this.peerConnections).map((peer) => {
            const { active, initiator, local, remote, error } = peer;
            return {
                active,
                initiator,
                local,
