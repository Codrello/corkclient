...
const messages = objectMap(import.meta.glob('../locales/*.json', { eager: true }), (path, component) => {
    const name = path.split('/').pop()?.slice(0, -5);
    return [name, cloneObject(component.default)];
});
...
/** Shortcut to $t */
export const t = i18n?.global?.t ? i18n.global.t.bind(i18n.global) : (s) => s;
// Locale
